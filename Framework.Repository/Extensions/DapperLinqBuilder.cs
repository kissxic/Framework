    ...
    public class DapperLinqBuilder<T> where T : class
    {
        ...
        private sealed class PredicateConverter
        {
            public static PredicateGroup Convert(Expression expression)
            {
                ...
                PredicateGroup group = result as PredicateGroup;
                if (group == null)
            ...
            private IPredicate ParseCallQueryFunction(MethodCallExpression expression)
            {
                ...
                {
                    ConstantExpression patternExpression = expression.Arguments[0] as ConstantExpression;
                    MemberExpression memberExpression = SimplifyExpression(expression.Arguments[1]) as MemberExpression;
            ...
            private IPredicate ParseCallEnumerableFunction(MethodCallExpression expression)
            {
                if (expression.Method.Name == "Contains")
                {
                    MemberExpression valueExpression = expression.Arguments[0] as MemberExpression;
                    MemberExpression memberExpression = SimplifyExpression(expression.Arguments[1]) as MemberExpression;
            ...
            private IPredicate ParseCallListFunction(MethodCallExpression expression)
            {
                ...
                {
                    MemberExpression valueExpression = expression.Arguments[0] as MemberExpression;
                    MemberExpression memberExpression = SimplifyExpression(expression.Object) as MemberExpression;
