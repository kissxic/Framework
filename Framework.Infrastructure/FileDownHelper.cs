...
namespace Framework.Infrastructure
{
    ...
    public class FileDownHelper
    {
        ...
        /// <param name="name">文件名(无扩展名无路径)</param>
        public static void DownLoad(byte[] buffer, string name = "文件导出列表")
        {
            long dataToRead = 0; //已读的字节数
            try
            {
                ...
                //添加Http头   
                HttpContext.Current.Response.ContentType = "application/octet-stream";
                string saveFileName = name + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss").Replace(":", "-").Replace(" ", "-") +
                               ".xls";
                HttpContext.Current.Response.AddHeader("Content-Disposition", "attachement;filename=" + HttpUtility.UrlEncode(saveFileName));
                HttpContext.Current.Response.AddHeader("Content-Length", dataToRead.ToString());
