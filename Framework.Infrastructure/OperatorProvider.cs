...
namespace Framework.Infrastructure
{
    ...
    public class OperatorProvider
    {
        ...
        /// </summary>
        private string LoginProvider = ConfigurationHelper.AppSetting("LoginProvider");

        /// <summary>
        /// 从配置文件读取登陆用户信息保存时间。
        /// </summary>
        private int LoginTimeout = Convert.ToInt32(ConfigurationHelper.AppSetting("LoginTimeout"));
        ...
        public Operator Current
        {
            get
            {
                ...
                {
                    operatorModel = WebHelper.GetCookie(LOGIN_USER_KEY).DESDecrypt().JsonToType<Operator>();
                }
                else
                {
                    operatorModel = WebHelper.GetSession(LOGIN_USER_KEY).DESDecrypt().JsonToType<Operator>();
                }
